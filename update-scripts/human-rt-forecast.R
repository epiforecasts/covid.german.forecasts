...
# set a min sample per person and a min target samples. Set the number of 
# samples per person to the taregt number / number of people
# obtain samples for every weekly forecast
# order samples
...
# Do EpiNow2 magic: 
# combine EpiNow2 model per region with expert samples and use 
# simulate_cases
...
## extrapolate quantiles in between
d <- forecast_quantiles %>%
  group_by(forecaster_id, region, quantile) %>%
  dplyr::mutate(difference = c(diff(value), 0)) %>%
