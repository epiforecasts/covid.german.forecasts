...
```{r}
ens <- ensemble_members[, .("GM_inc_case" = sum(GM_inc_case), 
                     "PL_inc_case" = sum(PL_inc_case), 
                     "GM_inc_death" = sum(GM_inc_death), 
                     "PL_inc_death" = sum(PL_inc_death)), 
                 by = "Date"] %>%
  melt(id.vars = "Date") 

ens[, .(mean = mean(value), 
       median = median(value)), by = "variable"]
...
ggsave("plots/distribution_scores_wis.png", pl_wis, height = 11, width = 9)
