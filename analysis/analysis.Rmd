...
truth_and_pred_plot <- function(df, ttype = "case") {
  ...
  limits <- min(forecast_dates$full_hub_period) + 5 + 1:21 * 7
...
distribution_plot_wis <- function(df, x = "interval_score", ranks = FALSE) {
  ...
  + runif(length(get(x)), min = -0.1, max = 0.1)
...
# function to make plot with mean of a given metric
plot_helper <- function(tmp = scores, xvar = x, y, rel = relative, 
                        facet_formula, colour) {
  if (rel) {
    tmp <- tmp %>%
...
# function for creating a component plot
component_plot <- function(scores, type, x = x, rel = relative,
                           facet_formula, vals = NULL) {
  
  if (type == "case" & is.null(vals)) {
    vals <- colcases
  } else if (type == "death" & is.null(vals)){
    vals <- coldeaths
  } else if (type == "case") {
    # filter out ensembles not needed
    scores <- scores %>%
      filter(!(Model %in% c("Hub-ensemble-with-all", "Hub-ensemble-with-convolution")))
    vals <- vals[-c(3, 4)]
  }
  
  scores <- scores %>%
    select(Model, target_type, sharpness, interval_score,
           underprediction, overprediction, !!x) %>%
    filter(target_type == type) %>%
    pivot_longer(names_to = "WIS components",
                 cols = c(sharpness, overprediction, underprediction)) %>%
    mutate(`WIS components` = fct_relevel(`WIS components`, 
                                          c("sharpness", "underprediction", 
                                            "overprediction"))) 
  ...
  if (rel) {
    scores <- scores %>%
      ...
      mutate_at("value", ~ . / interval_score[Model == "Hub-ensemble"])
  }
...
aggregate_performance <- function(data, 
  ...
  # mean performance
  p_mean <- 
    plot_helper(scores, y = "interval_score", rel = relative, 
                facet_formula = facet_formula, colour = vals) + 
    labs(y = "mean WIS", x = NULL)
  
    # absolute error of performance
  p_aem <-
    plot_helper(scores, y = "aem", rel = relative, 
                facet_formula = facet_formula, colour = vals) + 
    labs(y = "Absolute error", x = NULL) 
  
  # sd of performance
  p_wis_sd <- 
    plot_helper(scores, y = "interval_score_sd", rel = relative, 
                facet_formula = facet_formula, colour = vals) + 
    labs(y = "WIS - sd", x = NULL)
  
  # sharpness of performance
  p_sharpness <- 
    plot_helper(scores, y = "sharpness", rel = relative, 
                facet_formula = facet_formula, colour = vals) + 
  ...
  if (relative) {
    scores <- scores %>%
      ...
      mutate_at("coverage", ~ . - .[Model == "Hub-ensemble"])
  }
  ...
  p_combined2 <- (p_aem + p_wis_sd) /
    ...
    ggsave(here(output_dir, paste0(plotname, "-2.png")), 
         height = 5.1, width = 10)
    
    p_combined3 <- (p_mean + p_aem) /
        (p_cov50 + p_cov90) /
        (p_sharpness + p_bias) /
        (p_wis_sd + plot_spacer()) + 
    plot_layout(guides = "collect") & 
    plot_annotation(tag_levels = 'A')  &
    theme(legend.position = 'bottom', 
          legend.box="vertical", legend.margin=margin()) 

    ggsave(here(output_dir, paste0(plotname, "-3.png")), 
         height = 9.1, width = 10)
    ...
    ggsave(here(output_dir, paste0(plotname, "-4.png")), 
           height = 9.1, width = 10)
}
