...
truth_and_pred_plot <- function(df, ttype = "case") {
  ...
  p <- df %>%
    ...
    geom_linerange(aes(ymin = `0.25`, ymax = `0.75`, color = Model), 
                   size = 1.1,
                   alpha = 1,
                   position = position_dodge(width = 3.6)) + 
    ...
    scale_x_date(limits = c(min(forecast_dates$full_hub_period) + 5, 
                 ...
                 minor_breaks = forecast_dates$unfiltered + 5)
  return(p)
}
...
aggregate_performance <- function(data, 
  ...
  data <- filtered_data[model %in% regular_models]
  ...
  component_plot <- function(scores, type) {
    ...
    scores %>%
      select(Model, target_type, horizon, sharpness, interval_score,
             underprediction, overprediction) %>%
  ...
  component_case <- component_plot(scores, "case")
  component_death <- component_plot(scores, "death")
  ...
  p_cov50 <- scores[range %in% c(50)] %>%
    plot_helper(y = "coverage", rel = FALSE) + 
