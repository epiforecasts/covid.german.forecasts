...
df <- unfiltered_data[quantile %in% c(0.025, 0.25, 0.75, 0.975)] %>%
...
scores_over_time_plot <- function(scores, y = "mean_scores_ratio") {
  ...
  theme(legend.position = "bottom") +
  labs(y = "WIS relative to baseline", x = "Date") + 
...
for (horizon in horizons) {
  ...
  ggsave(here("analysis", "plots", paste0("figure-forecasts-", horizon, ".png")))
}
  ...
  for (i in horizons) {
    ...
    tmp_df <- df[target_phase == tar & horizon == i, 
                 .(model, interval_score, sharpness,
                   ...
                   coverage_deviation, relative_skill,  
