...
truth_vs_forecast_2_deaths <- df %>%
  ...
  geom_rect(aes(xmin = min(forecast_dates$full_hub_period) + 14,
...
p_combined <- ggplot(scores, aes(y = model, x = scaled_rel_skill, 
...
tbl <- scores_tables %>%
  merge(coverage_tables, all.x = TRUE, by = c("modellocation_target", "target_phase", "classification")) 
...
for (tar in targets) {
  ...
  table_dfs[[tar]] <- table_dfs[[tar]][, lapply(.SD, FUN = function(x) {
    if (is.numeric(x)) {
      return(round(signif(x, 3), 2))
    } else {
      return(x)
