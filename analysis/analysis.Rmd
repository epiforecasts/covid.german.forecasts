...
truth_and_pred_plot <- function(df, ttype = "case") {
  ...
  p <- df %>%
    ...
    geom_point(data = trend[target_type == ttype], size = 0.7) +
...
data <- unfiltered_data[model %in% c(regular_models, "Renewal-retrospective", "Convolution-retrospective")]

unfiltered_data$model %>% unique()
...
make_table <- function(data, 
  ...
  df <- df %>%
    ...
    dplyr::mutate_at(percentage_cols, 
                     ~ paste0(., " (", 
                              round(100 * ./.[model %in% c("Hub-ensemble", 
                                                           "Hub-ensemble-mean")]), 
                              "%)"))
  ...
  setnames(df, old = c("interval_score_sd"), 
           new = c("WIS - sd"))
...
filtered_data[model %in% regular_models] %>%
  ...
  make_table(plotname = "table_mean-late-period")
...
aggregate_performance <- function(data, 
  ...
  # mean performance
  p_mean <- 
    plot_helper(scores, y = "interval_score", rel = relative, xvar = x, 
  ...
  p_sharpness <- 
    plot_helper(scores, y = "sharpness", rel = relative, xvar = x,
                facet_formula = facet_formula, colour = vals) + 
