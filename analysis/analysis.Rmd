...
colcases <- brewer.pal(name = "Accent", n = 5)[-c(1, 4)]
coldeaths <- brewer.pal(name = "Accent", n = 5)[-4]
...
truth_and_pred_plot <- function(df, ttype = "case") {
  ...
  trend <- copy(epitrend)[, location_target := paste0(str_to_sentence(target_type), 
                                                    "s", " in ", location_name)]
  p <- df %>%
    ...
    # geom_point(data = unfiltered_data[target_type == ttype & horizon == 2]) + 
...
for (i in 1:4) {
  ...
  filename <- here("analysis", "plots", 
                   paste0("truth-and-", i, "-wk-ahead_cases"))
  ...
  filename <- here("analysis", "plots", 
                   paste0("truth-and-", i, "-wk-ahead_deaths"))
...
for (i in 1:4) {
  ...
  epi_tmp[, forecast_date := as.character(as.Date(forecast_date) + 7 * i)]
  epi_tmp <- epi_tmp[, .(location_name, forecast_date, classification, target_type)] %>% 
    unique()
  
  scores_over_time_case <- scores_target_forecastdate[horizon == i & target_type == "case"] %>%
    merge(epi_tmp) %>%
  ...
  scores_over_time_deaths <- scores_target_forecastdate[horizon == i & target_type == "death"] %>%
    merge(epi_tmp) %>%
    scores_over_time_plot(y = "interval_score") + 
    scale_color_manual(values = coldeaths) + 
    annotate('rect', xmin = min(forecast_dates$full_hub_period) + 14,
...
table <- make_score_table(df[horizon <= 2, !"target_type"], group = FALSE) %>%
...
scores <- eval_forecasts(filtered_data[model %in% regular_models], 
                         summarise_by = c("model", "target_type", "horizon"), 
                         quantiles = 0.5, 
                         sd = TRUE)

# mean performance
p_mean <- scores %>%
  ggplot(aes(x = horizon, y = interval_score, group = model, color = model)) + 
  geom_point() + 
  geom_line() + 
...
# ggsave(here("analysis", "plots", "aggregate-performance-mean.png"))
  

# median performance
p_median <- scores %>%
  ggplot(aes(x = horizon, y = interval_score_0.5, group = model, color = model)) + 
  geom_point() + 
  geom_line() + 
  facet_wrap(~ target_type, scales = "free_y") + 
  theme(legend.position = "bottom") + 
...
# ggsave(here("analysis", "plots", "aggregate-performance-median.png"))

# sd of performance
p_wis_sd <- scores %>%
  ggplot(aes(x = horizon, y = interval_score_sd, group = model, color = model)) + 
  geom_point() + 
  geom_line() + 
  facet_wrap(~ target_type, scales = "free_y") + 
  theme(legend.position = "bottom") + 
...
# ggsave(here("analysis", "plots", "aggregate-performance-wis-sd.png"))


# sharpness of performance
p_sharpness <- scores %>%
  ggplot(aes(x = horizon, y = sharpness, group = model, color = model)) + 
  geom_point() + 
  geom_line() + 
  facet_wrap(~ target_type, scales = "free_y") + 
  theme(legend.position = "bottom") + 
...
# ggsave(here("analysis", "plots", "aggregate-performance-sharpness.png"))

# absolute error of performance
p_aem <- scores %>%
  ggplot(aes(x = horizon, y = aem, group = model, color = model)) + 
  geom_point() + 
  geom_line() + 
  facet_wrap(~ target_type, scales = "free_y") + 
  theme(legend.position = "bottom") + 
...
# ggsave(here("analysis", "plots", "aggregate-performance-aem.png"))

# bias
p_bias <- scores %>%
  ggplot(aes(x = horizon, y = bias, group = model, color = model)) + 
  geom_point() + 
  geom_line() + 
  facet_wrap(~ target_type) + 
...
# ggsave(here("analysis", "plots", "aggregate-performance-bias.png"))

# coverage
scores <- eval_forecasts(filtered_data[model %in% regular_models], 
                         summarise_by = c("model", "target_type", "horizon", "range"), 
                         quantiles = 0.5, 
                         sd = TRUE)

scores[range %in% c(50, 90)] %>%
  ggplot(aes(x = horizon, y = coverage, group = range, color = range)) + 
  geom_point() + 
  geom_line() + 
  facet_wrap(model ~ target_type , scales = "free_y")

p_cov50 <- scores[range %in% c(50)] %>%
  ggplot(aes(x = horizon, y = coverage, group = model, color = model)) + 
  geom_point() + 
  geom_line() + 
  facet_wrap(~ target_type) + 
  geom_hline(yintercept = 0.5, linetype = "dashed") +
  scale_color_manual(values = coldeaths) + 
  labs(y = "Coverage - 50% PI", x = "Forecast horizon") + 
  theme_minimal() + 
...
# ggsave(here("analysis", "plots", "aggregate-performance-coverage50.png"))
...
p_cov90 <- scores[range %in% c(90)] %>%
  ...
  theme(legend.position = "bottom") + 
  scale_color_manual(values = coldeaths) + 
  labs(y = "Coverage - 90% PI", x = "Forecast horizon") + 
  expand_limits(y=c(0, 1)) + 
  scale_y_continuous(labels = scales::percent)

# ggsave(here("analysis", "plots", "aggregate-performance-coverage90.png"))
...
ggsave(here("analysis", "plots", "aggregate-performance-all.png"), 
       height = 9.1, width = 10)
...
### Aggregate performance across locations

```{r}
scores <- eval_forecasts(filtered_data[model %in% regular_models & horizon == 2], 
...
# mean performance
p_mean <- scores %>%
  ggplot(aes(x = location_name, y = interval_score, group = model, color = model)) + 
  geom_point() + 
...
# median performance
p_median <- scores %>%
  ggplot(aes(x = location_name, y = interval_score_0.5, group = model, color = model)) + 
  geom_point() + 
  geom_line(linetype = "dashed", size = 0.3) + 
  facet_wrap(~ target_type, scales = "free_y") + 
  theme(legend.position = "bottom") + 
...
# sd of performance
p_wis_sd <- scores %>%
  ggplot(aes(x = location_name, y = interval_score_sd, group = model, color = model)) + 
  geom_point() + 
  geom_line(linetype = "dashed", size = 0.3) + 
  facet_wrap(~ target_type, scales = "free_y") + 
  theme(legend.position = "bottom") + 
...
# sharpness of performance
p_sharpness <- scores %>%
  ggplot(aes(x = location_name, y = sharpness, group = model, color = model)) + 
  geom_point() + 
  geom_line(linetype = "dashed", size = 0.3) + 
  facet_wrap(~ target_type, scales = "free_y") + 
  theme(legend.position = "bottom") + 
...
# absolute error of performance
p_aem <- scores %>%
  ggplot(aes(x = location_name, y = aem, group = model, color = model)) + 
  geom_point() + 
  geom_line(linetype = "dashed", size = 0.3) + 
  facet_wrap(~ target_type, scales = "free_y") + 
  theme(legend.position = "bottom") + 
...
# bias
p_bias <- scores %>%
  ggplot(aes(x = location_name, y = bias, group = model, color = model)) + 
  geom_point() + 
  geom_line(linetype = "dashed", size = 0.3) + 
  facet_wrap(~ target_type) + 
  geom_hline(yintercept = 0, linetype = "dashed") +
...
# coverage
scores <- eval_forecasts(filtered_data[model %in% regular_models], 
                         summarise_by = c("model", "target_type", "location_name", "range"), 
                         quantiles = 0.5, 
                         sd = TRUE)

p_cov50 <- scores[range %in% c(50)] %>%
  ggplot(aes(x = location_name, y = coverage, group = model, color = model)) + 
  geom_point() + 
  geom_line(linetype = "dashed", size = 0.3) + 
  facet_wrap(~ target_type) + 
  geom_hline(yintercept = 0.5, linetype = "dashed") +
  scale_color_manual(values = coldeaths) + 
  labs(y = "Coverage - 50% PI", x = "Location") + 
  theme_minimal() + 
  expand_limits(y=c(0, 1)) + 
  theme(legend.position = "bottom") + 
  scale_y_continuous(labels = scales::percent)
...
p_cov90 <- scores[range %in% c(90)] %>%
  ...
  theme(legend.position = "bottom") + 
  scale_color_manual(values = coldeaths) + 
  labs(y = "Coverage - 90% PI", x = "Location") + 
  expand_limits(y=c(0, 1)) + 
  scale_y_continuous(labels = scales::percent)
...
ggsave(here("analysis", "plots", "aggregate-performance-2-weeks-locations-all.png"), 
       height = 9.1, width = 10)
...
```{r}
scores <- eval_forecasts(filtered_data[model %in% 
                                         ensemble_models[!grepl("mean", ensemble_models)]], 
                         summarise_by = c("model", "target_type", "horizon"), 
                         quantiles = 0.5, 
                         sd = TRUE)
...
p_mean_ens <- scores %>%
  ...
  theme(legend.position = "bottom") + 
  scale_color_manual(values = coldeaths) + 
  labs(y = "mean WIS", x = "Forecast horizon")
...
scores <- scores[order(interval_score), , by = c("target_type", "location", 
                                               "horizon", "forecast_date")]
scores <- scores[, ranking := 1:.N, by = c("target_type", "location", 
                                           "horizon", "forecast_date")]
