...
Good to have you as a forecaster! You're receiving this message because you gave us your email address when you registered for the [Epiforecasts Crowd Forecast App](https://cmmid-lshtm.shinyapps.io/crowd-forecast/). 

It has been a long time, but finally we managed to get this email newsletter running! From now on you will receive a weekly email with

- announcements and other information
- a reminder to submit a forecast
- some information about last weeks forecast and your performance

If you don't want to receive future emails simply write me a quick message. We're working on a way that allows you to unsubscribe from within the app, but that is work in progress. Baby steps, but we're getting there. 
...
```{r plot-forecasts, echo=FALSE, out.width="49%", fig.show="hold", fig.cap = "Plot of the forecasts"}

forecast_date <- covid.german.forecasts::latest_weekday(Sys.Date())

filter_truth <- list('target_end_date >= Sys.Date() - 14 * 7')
models <- c("anonymous_Human", "EpiExpert-ensemble")
...
```{r rank-over-time, echo=FALSE, fig.align = "center", fig.width=10, fig.height=8, out.width="80%", fig.cap = "Ranking over time"}
...
If you have any questions or feedback, please don't hesitate to reach out. You can drop us an email or submit an issue to our [github repo](https://github.com/epiforecasts/covid.german.forecasts)
