...
<!-- In addition to the official Hub ensembles, we computed versions of the  -->
...
We used two models from the EpiNow2 R package (version 1.3.3) as our model-based forecasts (Abbott, Hellewell, et al. 2020). The first of these models, here called “renewal model”, uses the renewal equation [@fraserEstimatingIndividualHousehold2007] to predict reported cases and deaths. It estimates the effective reproduction number $R_t$ (the average number of people each infected person is expected to infect in turn) and models future infections as a weighted sum of past infections times $R_t$. $R_t$ was assumed to stay constant in the future, roughly corresponding to continuing the latest trend in cases. The $R_t$ value which was carried forward was slightly changed on November 9th 2020, where before that date the last 'reliable' $R_t$ estimate from X days before the forecast was used and after that the latest and most up-to-date (but possibly less stable) $R_t$ estimate made on the actual forecast date was used. Reported cases and deaths were obtained by convolving predicted infections over data-based delay distributions (Abbott, Hellewell, et al. 2020; DOI n.d.; “Evaluating the Use of the Reproduction Number as an Epidemiological Tool, Using Spatio-Temporal Trends of the Covid-19 Outbreak in England | medRxiv” n.d.) to model the time between infection and report date or death. The renewal model was used to predict cases as well as deaths, but the two were forecast completely indepdently, meaning that $R_t$ was forecast once from reported cases and once from reported deaths. Death forecasts from the renewal model were therefore not informed by past cases. 

The second model (“convolution model”) was only used to forecast deaths and modeled these as a fraction of infected people who would die with some delay. 

Both models were left completely untuned and unchanged throughout the study period to create a baseline purely informed by epidemiological assumptions. Human forecasters, on the other hand, were able to adapt their forecasts to current or likely future interventions.



The second model, here called “convolution model”, which was only used to forecast deaths, assumed that deaths could be modelled using a scaling parameter, as a  convolution of test positive cases with a distribution that described the delay from case report to death and a scaling factor (the case-fatality ratio). Both models assumed a negative binomial observation model and a multiplicative day-of-the-week effect (Abbott, Hellewell, et al. 2020). 

All model fitting was done using Markov-chain Monte Carlo (MCMC) in stan (Stan Development Team 2020) with each location and forecast target being fit separately. More details are available in the supplementary information. 


Line list data used to inform the delay from symptom onset to test positive case report or death in the model-based forecasts was sourced from (Xu et al., n.d.) with data available up to the 1st of August. Population data at the national and state level in Germany and Poland used in the model-based forecasts was sourced from (Statistisches Bundesamt 2020) and (Statystyczny 2020).
...
Crowd forecasts and the renewal model were created over the whole period between October 12th 2020 until March 1st 2021. We added the convolution model later and started creating and submitting it on December 7th 2020. As the first submission suffered from a software bug, we disregard it here. 
Crowd forecasts and renewal model forecasts were created in real time over a period of 21 weeks from October 12th 2020 until March 1st 2021. We added the convolution model later and started creating it on December 7th 2020. As the first forecast suffered from a software bug, we disregard it here. All our forecasts were time-stamped and submitted to the German and Polish Forecast Hub (CITATION). 
All forecasts were processed in a Docker (Merkel 2014) container to ensure a reproducible environment.


To ensure compatibility between models, all death forecasts were scored using only the period from December 14th on, whereas case forecasts were evaluated on the entire submission period.
They were both submitted to the so-called German/Polish forecast hub (http ref) and thus time-stamped (with one belated submission for the renewal model on December 28). 
...
We used two models from the EpiNow2 R package (version 1.3.3) as our model forecasts (Abbott, Hellewell, et al. 2020). The first of these models, here called “renewal model”, which was used to forecast both test positive cases and deaths, used the renewal equation [@fraserEstimatingIndividualHousehold2007] to estimate the effective reproduction number over time for latent infections and then convolved these infections to the target observation using data based delay distributions (Abbott, Hellewell, et al. 2020; DOI n.d.; “Evaluating the Use of the Reproduction Number as an Epidemiological Tool, Using Spatio-Temporal Trends of the Covid-19 Outbreak in England | medRxiv” n.d.). Line list data used to inform the delay from symptom onset to test positive case report or death in the model-based forecasts was sourced from (Xu et al., n.d.) with data available up to the 1st of August. For the forecasts, the reproduction number was assumed to remain constant from X days before the last data for which case data was available, roughly corresponding to continuing the latest trend in cases. 

The second model, here called “convolution model”, which was only used to forecast deaths, assumed that deaths could be modelled using a scaling parameter, as a  convolution of test positive cases with a distribution that described the delay from case report to death and a scaling factor (the case-fatality ratio). Both models assumed a negative binomial observation model and a multiplicative day-of-the-week effect (Abbott, Hellewell, et al. 2020). All model fitting was done using Markov-chain Monte Carlo (MCMC) in stan (Stan Development Team 2020) with each location and forecast target being fit separately. More details are available in the supplementary information. 
