...
%DIF ADD manuscript/manuscript.tex                             Mon Aug 29 14:59:14 2022
...
\ref{fig:daily-truth-update} and \ref{fig:weekly-truth-update}}\DIFdelend \DIFaddbegin \nameref{fig:daily-truth-update} \DIFadd{and }\nameref{fig:weekly-truth-update}
...
\DIFdelbegin \DIFdel{Figure \ref{fig:screenshot} in the SI}\DIFdelend \DIFaddbegin \nameref{fig:screenshot}\DIFaddend ). The default shown was a repetition of the
...
\DIFdelbegin \DIFdel{the SI}\DIFdelend \DIFaddbegin \nameref{txt:details-models}\DIFaddend ). It estimated the effective reproduction
...
\DIFdelbegin \DIFdel{SI}\DIFdelend \DIFaddbegin \nameref{txt:details-models}\DIFaddend ) was only used to forecast deaths and was
...
\ref{tab:scoring-metrics} in the SI}\DIFdelend \DIFaddbegin \nameref{tab:scoring-metrics}\DIFaddend . All scores were calculated using the
...
\DIFadd{and }\nameref{fig:num-ensemble-members}\DIFaddend ). What we call ``Hub ensemble''
...
}\DIFaddend \ref{fig:agg-performance-all}A) and locations
(\DIFdelbegin \DIFdel{Figure
\ref{fig:performance-locations-rel}A }\DIFdelend \DIFaddbegin \nameref{fig:performance-locations-rel}\DIFadd{A Fig}\DIFaddend ). For two week ahead
forecasts, mean WIS relative to the Hub ensemble (= 1) was 0.89 for
crowd forecasts and 1.40 for the renewal model
(\DIFdelbegin \DIFdel{Table \ref{tab:score-table-2}}\DIFdelend \DIFaddbegin \nameref{tab:score-table-2} \DIFadd{Table}\DIFaddend ). Across all forecasting approaches,
...
outliers\DIFdelbegin \DIFdel{(Figure
\ref{fig:distribution-scores})}\DIFdelend . Overall, low variance in forecast performance was closely
linked with good mean performance (\DIFdelbegin \DIFdel{Figures
}\DIFdelend \DIFaddbegin \DIFadd{Fig }\DIFaddend \ref{fig:agg-performance-all}H
and \DIFdelbegin \DIFdel{and }\DIFdelend \ref{fig:agg-performance-all}A), suggesting that the ability to
...
the renewal model, which had more outliers\DIFdelbegin \DIFdel{(Figure \ref{fig:distribution-scores})}\DIFdelend , as well as the highest
...
four weeks ahead) (\DIFdelbegin \DIFdel{Figure \ref{fig:agg-performance-all}C ,D and Tables
\ref{tab:score-table-2} and \ref{tab:score-table-4}}\DIFdelend \DIFaddbegin \DIFadd{Fig \ref{fig:agg-performance-all}C and
\ref{fig:agg-performance-all}D as well as }\nameref{tab:score-table-2}
\DIFadd{and }\nameref{tab:score-table-4}\DIFadd{s}\DIFaddend ). Despite worse performance in terms of
absolute error (\DIFdelbegin \DIFdel{Figure
}\DIFdelend \DIFaddbegin \DIFadd{Fig }\DIFaddend \ref{fig:agg-performance-all}G), the renewal model
...
\ref{fig:performance-locations-rel}}\DIFdelend \DIFaddbegin \nameref{fig:performance-locations} \DIFadd{and
}\nameref{fig:performance-locations-rel} \DIFadd{Figs}\DIFaddend ). Due to the differing
...
\ref{fig:agg-performance-all-Poland}}\DIFdelend \DIFaddbegin \nameref{fig:performance-locations-rel}\DIFadd{A,
}\nameref{fig:performance-locations-rel}\DIFadd{G,
}\nameref{fig:agg-performance-all-Germany} \DIFadd{and
}\nameref{fig:agg-performance-all-Poland} \DIFadd{Figs}\DIFaddend ).
...
Tables \ref{tab:score-table-2}, \ref{tab:score-table-4}}\DIFdelend \DIFaddbegin \DIFadd{Fig
...
\DIFdelbegin \DIFdel{Table
\ref{tab:table-ensemble-versions} in the SI}\DIFdelend \DIFaddbegin \nameref{tab:table-ensemble-versions}\DIFaddend .
...
%DIFDELCMD < %%%
\DIFdel{We thank all forecasters who participated in this study for their
contribution.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We would also like to acknowledge (in a randomised order) the members of
Centre for the Mathematical Modelling of Infectious Diseases COVID-19
...
%DIFDELCMD < \setcounter{table}{0}  \renewcommand{\thetable}{S\arabic{table}} \setcounter{figure}{0} \renewcommand{\thefigure}{S\arabic{figure}}
%DIFDELCMD < 

%DIFDELCMD < \hypertarget{appendix-supplementary-information}{%
...
%DIFDELCMD < \hypertarget{supplementary-information}{%
%DIFDELCMD < \section{Supplementary information}\label{supplementary-information}}
%DIFDELCMD < 

%DIFDELCMD < \hypertarget{scoring-metrics-used}{%
%DIFDELCMD < \subsection{Scoring metrics used}\label{scoring-metrics-used}}
%DIFDELCMD < 

%DIFDELCMD < \begin{longtable}[t]{>{\raggedright\arraybackslash}p{2.5cm}>{\raggedright\arraybackslash}p{9.3cm}}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
%DIFDELCMD < \cellcolor{gray!6}{In contrast to the over- and underprediction penalties of the interval score it is bound between 0 and 1 and represents a general tendency of forecasts to be biased rather than the absolute amount of over- and underprediction. It is therefore a more robust measurement.}\\*
...
%DIFDELCMD < \hypertarget{the-crowdforecasting-app}{%
...
bootstrap using 1\% or 1769 samples of the available data }%DIFDELCMD < {[}%%%
...
%DIFDELCMD < \hypertarget{model-fitting}{%
%DIFDELCMD < \subsubsection{Model fitting}\label{model-fitting}}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Both models were implemented using the }\texttt{\DIFdel{EpiNow2}} %DIFAUXCMD
\DIFdel{R package
(version 1.3.3) }%DIFDELCMD < {[}%%%
\DIFdel{40}%DIFDELCMD < {]}%%%
...
%DIFDELCMD < \label{fig:agg-performance-all-Poland}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < \hypertarget{performance-across-locations-in-absolute-terms}{%
...
%DIFDELCMD < \label{fig:performance-locations}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < \hypertarget{performance-across-locations-in-relative-terms}{%
...
%DIFDELCMD < \label{fig:performance-locations-rel}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
%DIFDELCMD < \hypertarget{visualisation-of-daily-reported-cases-and-deaths}{%
...
%DIFDELCMD < \label{fig:daily-truth}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
%DIFDELCMD < \label{fig:daily-truth-update}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
%DIFDELCMD < \label{fig:weekly-truth-update}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
%DIFDELCMD < \label{fig:forecasts-and-truth-1}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
%DIFDELCMD < \label{fig:forecasts-and-truth-3}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
%DIFDELCMD < \label{fig:forecasts-and-truth-4}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
%DIFDELCMD < \hypertarget{distribution-of-scores}{%
%DIFDELCMD < \subsection{Distribution of scores}\label{distribution-of-scores}}
%DIFDELCMD < 

%DIFDELCMD < \hypertarget{absolute-scores}{%
...
%DIFDELCMD < \hypertarget{ranks-achieved-by-forecasts}{%
...
%DIFDELCMD < \label{fig:distribution-scores-ranks-1}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
%DIFDELCMD < \label{fig:distribution-scores-differences}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
%DIFDELCMD < \label{fig:distribution-scores-differences-renewal}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
%DIFDELCMD < \hypertarget{comparison-of-ensembles}{%
%DIFDELCMD < \subsection{Comparison of ensembles}\label{comparison-of-ensembles}}
%DIFDELCMD < 

%DIFDELCMD < \hypertarget{performance-visualisation-mean-ensemble}{%
...
%DIFDELCMD < \label{fig:agg-performance-ensemble-mean}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < \hypertarget{tables-median-ensemble}{%
...
%DIFDELCMD < \hypertarget{tables-mean-ensemble}{%
...
%DIFDELCMD < \hypertarget{sensitivity-analysis}{%
...
%DIFDELCMD < \hypertarget{overview-of-models-and-forecasters}{%
%DIFDELCMD < \subsection{Overview of models and
%DIFDELCMD < forecasters}\label{overview-of-models-and-forecasters}}
%DIFDELCMD < 

%DIFDELCMD < \begin{longtable}[t]{>{\raggedright\arraybackslash}p{4.5cm}>{\raggedright\arraybackslash}p{7.3cm}}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
\DIFdel{Official Forecast Hub mean ensemble. Created by the Forecast Hub officially under the name 'KITCOVIDhub-mean\_ensemble'.}%DIFDELCMD < \\
%DIFDELCMD < \addlinespace \addlinespace
%DIFDELCMD < \cellcolor{gray!6}{ \vphantom{1}} & \cellcolor{gray!6}{}\\
%DIFDELCMD < \addlinespace \addlinespace
%DIFDELCMD < %%%
\DIFdel{Hub-ensemble }%DIFDELCMD < & %%%
\DIFdel{Version of the official Hub median ensemble which excludes all our contributions.}%DIFDELCMD < \\
%DIFDELCMD < \addlinespace \addlinespace
...
\DIFdel{Hub-ensemble-with-renewal, 
    Hub-ensemble-with-renewal-mean }%DIFDELCMD < & %%%
...
}%DIFDELCMD < \cellcolor{gray!6}{    Hub-ensemble-with-crowd-mean} & \cellcolor{gray!6}{Versions of the official Hub ensembles which of our contributions includes only the Crowd forecast.}\\
%DIFDELCMD < \addlinespace \addlinespace
%DIFDELCMD < %%%
\DIFdel{Hub-ensemble-with-convolution, 
    Hub-ensemble-with-convolution-mean }%DIFDELCMD < & %%%
\DIFdel{Versions of the official Hub ensembles which of our contributions includes only the Convolution model (which originally was never included in any official Hub ensemble).}%DIFDELCMD < \\
%DIFDELCMD < \addlinespace \addlinespace
%DIFDELCMD < %%%
\DIFdel{Hub-ensemble-with-all, 
}%DIFDELCMD < \cellcolor{gray!6}{    Hub-ensemble-with-all-mean} & \cellcolor{gray!6}{Versions of the official Hub ensembles which includes all our contributions. For cases, this is identical to the official Hub ensembles, but for deaths the convolution model was added.}\\
%DIFDELCMD < \addlinespace \addlinespace
%DIFDELCMD <  & \\
%DIFDELCMD < \addlinespace \addlinespace
...
\DIFdel{Submitted to the Forecast Hub as 'epiforecasts-EpiNow2'}%DIFDELCMD < \\
%DIFDELCMD < \addlinespace \addlinespace
...
%DIFDELCMD < \hypertarget{comparison-of-crowd-forecasts-and-application-baseline}{%
...
%DIFDELCMD < 2020;25. }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \end{CSLReferences}
\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{We thank all forecasters who participated in this study for their
contribution.
}\DIFaddend 
...
\textbf{\DIFadd{Overview of the scoring metrics used.}}

\paragraph*{\DIFadd{S1 Text.}}
\label{txt:details-models}

\textbf{\DIFadd{Further details on the semi-mechanistic forecasting models.}}
...
\textbf{\DIFadd{Scores for three and four weeks ahead forecasts.}} \DIFadd{Scores are cut
...
\textbf{\DIFadd{Fig.}} \label{fig:agg-performance-all-Germany}
...
\textbf{\DIFadd{Fig.}} \label{fig:agg-performance-all-Poland}
\textbf{\DIFadd{Visualisation of aggregate performance metrics for forecasts one to four weeks into the future in Poland.}}
\DIFadd{A, B: mean weighted interval score (WIS, lower indicates better
performance) across horizons. WIS is decomposed into its components
dispersion, over-prediction and under-prediction. C: Empirical coverage
of the 50\% prediction intervals (50\% coverage is perfect). D:
...
\textbf{\DIFadd{Fig.}} \label{fig:performance-locations}
\textbf{\DIFadd{Visualisation of aggregate performance metrics across locations.}}
\DIFadd{A, B: mean weighted interval score (WIS, lower indicates better
performance) across horizons. WIS is decomposed into its components
dispersion, over-prediction and under-prediction. C: Empirical coverage
of the 50\% prediction intervals (50\% coverage is perfect). D:
...
\textbf{\DIFadd{Fig.}} \label{fig:performance-locations-rel}
\textbf{\DIFadd{Visualisation of aggregate performance metrics across locations in relative terms.}}
...
\textbf{\DIFadd{Fig.}} \label{fig:daily-truth}
\textbf{\DIFadd{Visualisation of daily report data.}} \DIFadd{The black line represents
...
\textbf{\DIFadd{Fig.}} \label{fig:daily-truth-update}
\textbf{\DIFadd{Visualisation of the absolute difference between the daily report data at the time and the data now.}}
...
\textbf{\DIFadd{Fig.}} \label{fig:weekly-truth-update}
\textbf{\DIFadd{Visualisation of the relative difference between the weekly report data at the time and the data now.}}
...
\textbf{\DIFadd{Visualisation of forecasts and scores for one week ahead forecasts.}}
\DIFadd{A, C: Visualisation of 50\% prediction intervals of one week ahead
...
\textbf{\DIFadd{Visualisation of forecasts and scores for three week ahead forecasts.}}
\DIFadd{A, C: Visualisation of 50\% prediction intervals of three week ahead
forecasts against the reported values. Forecasts that were not scored
(because there was no complete set of death forecasts available) are
greyed out. B, D: Visualisation of corresponding WIS.
}
...
\textbf{\DIFadd{Distribution of weighted interval scores for one week ahead forecasts}}
...
\textbf{\DIFadd{Distribution of weighted interval scores for three week ahead forecasts}}
...
\textbf{\DIFadd{Distribution of weighted interval scores for four week ahead forecasts}}
...
\textbf{\DIFadd{Distribution of model ranks (in terms of WIS) for one week ahead forecasts}}
...
\textbf{\DIFadd{Distribution of model ranks (in terms of WIS) for two week ahead forecasts}}
\DIFadd{A: Distribution of the ranks (determined by the weighted interval score)
...
\textbf{\DIFadd{Distribution of model ranks (in terms of WIS) for three week ahead forecasts}}
\DIFadd{A: Distribution of the ranks (determined by the weighted interval score)
...
\textbf{\DIFadd{Distribution of model ranks (in terms of WIS) for four week ahead forecasts}}
\DIFadd{A: Distribution of the ranks (determined by the weighted interval score)
...
\textbf{\DIFadd{Difference in WIS between the Crowd forecast and the Hub ensemble for two week ahead forecasts.}}
...
\textbf{\DIFadd{Difference in WIS between the Crowd forecast and the Renewal model for two week ahead forecasts.}}
\DIFadd{Values below zero mean better performance of the Crowd forecasts.
}

\paragraph{\DIFadd{S21}}

\textbf{\DIFadd{Fig.}} \label{fig:agg-performance-ensemble-mean}
\textbf{\DIFadd{Visualisation of aggregate performance metrics across forecast horizons for the different versions of the Hub mean ensemble.}}
...
\textbf{\DIFadd{Table.}} \label{tab:score-table-ensemble-2}
\textbf{\DIFadd{Scores for one and two week ahead forecasts for the different versions of the median ensemble.}}
...
\textbf{\DIFadd{Scores for three and four week ahead forecasts for the different versions of the median ensemble.}}
\DIFadd{Scores are cut to three significant digits and rounded. Note that scores
for cases (which include the whole period from October 12th 2020 until
March 1st 2021) and deaths (which include only forecasts from the 21st
of December 2020 on) are computed on different subsets. Numbers in
...
\textbf{\DIFadd{Scores for one and two week ahead forecasts for the different versions of the mean ensemble}}
\DIFadd{Scores are cut to three significant digits and rounded. Note that scores
for cases (which include the whole period from October 12th 2020 until
March 1st 2021) and deaths (which include only forecasts from the 21st
of December 2020 on) are computed on different subsets. Numbers in
...
\textbf{\DIFadd{Scores for three and four week ahead forecasts for the different versions of the mean ensemble}}
\DIFadd{Scores are cut to three significant digits and rounded. Note that scores
for cases (which include the whole period from October 12th 2020 until
March 1st 2021) and deaths (which include only forecasts from the 21st
of December 2020 on) are computed on different subsets. Numbers in
...
\paragraph{\DIFadd{S22 Fig.}}
\label{fig:agg-performance-all-late}

\textbf{\DIFadd{Visualisation of aggregate performance metrics across forecast horizons (period from December 14th 2020 on)}}
...
\textbf{\DIFadd{Table.}} \label{tab:score-table-late-2}
...
\textbf{\DIFadd{Overview of the models and ensembles used.}}

\paragraph{\DIFadd{S23 Fig.}}
\label{fig:num-forecasters}

\textbf{\DIFadd{Number of participants who submitted a forecast over time.}}

\paragraph{\DIFadd{S24 Fig.}}
\label{fig:num-ensemble-members}

\textbf{\DIFadd{Number of member models in the official Hub ensemble}} \DIFadd{This
...
\paragraph{\DIFadd{S25 Fig.}}
\label{fig:compare-forecasters}

\textbf{\DIFadd{Crowd forecasts and baseline shown in the application for a two week horizon.}}
...
\paragraph*{\DIFadd{S1 Acknowledgements.}}

\textbf{\DIFadd{Members of the CMMID COVID-19 working group}}
\DIFaddend 
