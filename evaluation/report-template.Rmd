...
Here is an overall ranking of all forecasters. The ranking is made according to relative skill. Relative skill is calculated by looking at all pairwise comparisons between forecasters in terms of the weighted interval score (WIS). See below for a more detailed explanation of the scoring metrics used. 
...
forecast_dates <- as.character(unique(data$forecast_date[!is.na(data$forecast_date)]))
...
This table gives the rank of every forecaster among all forecasters for a given target and forecast date (1 is the best). Ranks are determined based on the weighted interval scores.
