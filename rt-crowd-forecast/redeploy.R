...
saveRDS(submission_date, here("rt-crowd-forecast", "data", "submission_date.RDS"))
first_forecast_date <- as.character(as.Date(submission_date) - 16)

# copy Rt data into app
obs_filt_rt_cases <- fread(here("rt-forecast", "data", "summary", "cases", submission_date, "rt.csv")) %>%
...
obs_filt_rt_deaths <- fread(here("rt-forecast", "data", "summary", "deaths", submission_date, "rt.csv")) %>%
  rename(value = median, target_end_date = date) %>%
...
rsconnect::deployApp(appDir = here("rt-crowd-forecast"), 
